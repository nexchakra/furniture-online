    ...
    <main className="h-screen w-full bg-[#f8f9fa] flex flex-col overflow-hidden font-sans relative">
          ...
          <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              ...
              <div className="flex gap-3">
                <button onClick={downloadComparison} className="px-6 py-2.5 bg-indigo-600 text-white rounded-full font-black text-[9px] uppercase tracking-widest hover:bg-slate-900 transition-all shadow-md">Export PDF</button>
                <button onClick={() => setShowSpecTable(false)} className="w-11 h-11 rounded-full bg-white shadow-md flex items-center justify-center font-bold hover:bg-rose-50 hover:text-rose-500 transition-colors border border-slate-100">✕</button>
              </div>
            </div>
            <div className="p-10">
      ...
      <header className="h-24 bg-white border-b border-slate-200 flex items-center justify-between px-12 z-30 shadow-sm">
        ...
        <div className="flex items-center gap-4 bg-slate-50 p-2.5 rounded-2xl border border-slate-100 shadow-inner">
          <button onClick={() => setSyncRotation(!syncRotation)} className={`px-8 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${syncRotation ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100' : 'bg-white text-slate-400 border border-slate-200'}`}>
            {syncRotation ? '🔗 Rotation Synced' : '🔓 Independent View'}
          </button>
          <div className="flex gap-1.5 pr-2">
            <button onClick={() => handleRotate('left')} className="w-11 h-11 bg-white rounded-xl shadow-sm font-black hover:bg-indigo-50 border border-slate-100 active:scale-90 transition-transform">⟲</button>
            <button onClick={() => handleRotate('right')} className="w-11 h-11 bg-white rounded-xl shadow-sm font-black hover:bg-indigo-50 border border-slate-100 active:scale-90 transition-transform">⟳</button>
          </div>
        </div>
      </header>
      ...
      <section className="flex-1 flex gap-[2px] bg-slate-200 relative shadow-inner">
        ...
        <div className="flex-1 bg-white relative group">
          <div className="absolute top-10 left-10 z-10 space-y-2 pointer-events-none">
            ...
            <h2 className="text-4xl font-black italic text-slate-900 uppercase tracking-tighter italic">{leftConfig.texture} Pro</h2>
          </div>
          ...
          <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-slate-900/90 backdrop-blur-xl p-4 rounded-[25px] shadow-2xl border border-white/10 opacity-0 group-hover:opacity-100 transition-all duration-500 z-50">
             ...
             <div className="flex gap-2.5 border-l pl-4 border-white/20">
                {PREMIUM_FINISHES.map(f => (
                  <button key={f.hex} onClick={() => setLeftConfig({...leftConfig, color: f.hex})} className={`w-7 h-7 rounded-full border-2 transition-all ${leftConfig.color === f.hex ? 'border-white scale-110 shadow-lg' : 'border-transparent hover:scale-105'}`} style={{ backgroundColor: f.hex }} />
                ))}
             </div>
          </div>
        </div>
        ...
        <div className="flex-1 bg-white relative group">
          <div className="absolute top-10 right-10 z-10 text-right space-y-2 pointer-events-none">
            ...
            <h2 className="text-4xl font-black italic text-slate-900 uppercase tracking-tighter italic">{rightConfig.back} Core</h2>
          </div>
          ...
          <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-slate-900/90 backdrop-blur-xl p-4 rounded-[25px] shadow-2xl border border-white/10 opacity-0 group-hover:opacity-100 transition-all duration-500 z-50">
             ...
             <div className="flex gap-2.5 border-l pl-4 border-white/20">
                {PREMIUM_FINISHES.map(f => (
                  <button key={f.hex} onClick={() => setRightConfig({...rightConfig, color: f.hex})} className={`w-7 h-7 rounded-full border-2 transition-all ${rightConfig.color === f.hex ? 'border-white scale-110 shadow-lg' : 'border-transparent hover:scale-105'}`} style={{ backgroundColor: f.hex }} />
                ))}
             </div>
          </div>
        </div>

        <button onClick={() => setShowSpecTable(true)} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40 group hover:scale-110 transition-transform"><div className="w-20 h-20 bg-slate-900 text-white rounded-full flex flex-col items-center justify-center shadow-2xl border-4 border-white group-hover:bg-indigo-600 transition-colors"><span className="font-black italic text-sm leading-none">VS</span><span className="text-[7px] font-black uppercase tracking-[0.2em] mt-1.5 opacity-60">Logic</span></div></button>
      </section>
      ...
      <footer className="h-40 bg-white border-t border-slate-200 grid grid-cols-2 px-12 divide-x divide-slate-100 shadow-2xl z-20">
        <div className="flex items-center justify-between pr-10">
          ...
          <button onClick={() => setFinalizePopup({show: true, price: priceA})} className="bg-indigo-600 text-white py-5 px-10 rounded-[20px] font-black text-[11px] uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl active:scale-95">Finalize Variant A</button>
        </div>
        <div className="flex items-center justify-between pl-10">
          ...
          <button onClick={() => setFinalizePopup({show: true, price: priceB})} className="border-2 border-slate-900 text-slate-900 py-5 px-10 rounded-[20px] font-black text-[11px] uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all active:scale-95">Finalize Variant B</button>
        </div>
      </footer>
    </main>
  );
